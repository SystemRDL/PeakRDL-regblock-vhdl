...
from ..utils import VhdlInt, do_bitswap, do_slice, get_vhdl_type
...
class ReadbackMuxGenerator(RDLForLoopGenerator):
    ...
    def process_external_block(self, node: AddressableNode) -> None:
        ...
        self.add_content(f"if rd_mux_addr >= {addr_lo} and rd_mux_addr <= {addr_hi} then")
        data = self.exp.hwif.get_external_rd_data(node)
        self.add_content(f"    readback_data_var := {data};")
        self.add_content("end if;")
    ...
    def process_external_reg(self, node: RegNode) -> None:
        ...
        if regwidth > accesswidth:
            ...
            for subword_idx in range(n_subwords):
                ...
                self.add_content(f"if {conditional} then")
            ...
            if regwidth < self.exp.cpuif.data_width:
                self.add_content(f"    {var}({regwidth-1} downto 0) := {data};")
            else:
                self.add_content(f"    {var} := {data};")
            self.add_content("end if;")
    ...
    def process_reg(self, node: RegNode, fields: Sequence[FieldNode]) -> None:
        ...
        for field in fields:
            ...
            if self.exp.hwif.has_value_input(field) and not field.implements_storage:
                ...
                if field.width == 1:
                    if get_vhdl_type(field) != "std_logic":
                        value = f"to_std_logic({value})"
                else:
            ...
            if field.width == 1:
                ...
                self.add_content(f"    {var}({field.high} downto {field.low}) := {value};")
    ...
    def process_buffered_reg(self, node: RegNode, regwidth: int, accesswidth: int) -> None:
        ...
        if accesswidth < regwidth:
            ...
            for subword_idx in range(n_subwords):
                ...
                conditional = self.get_addr_compare_conditional(addr)
                var = self.get_readback_data_var(addr)
                bslice = f"({(subword_idx + 1) * accesswidth - 1} downto {subword_idx*accesswidth})"
                self.add_content(f"if {conditional} then")
            ...
            var = self.get_readback_data_var(addr)
            self.add_content(f"if {conditional} then")
            self.add_content(f"    {var}({regwidth-1} downto 0) := {rbuf};")
            self.add_content("end if;")
    ...
    def get_wide_reg_subword_assignments(self, node: RegNode, fields: Sequence[FieldNode], regwidth: int, accesswidth: int) -> List[List[str]]:
        ...
        for field in fields:
            ...
            if field.high < accesswidth * (subword_idx + 1):
                ...
                var = self.get_readback_data_var(addr)
                if field.width == 1:
                    subword_assignments[subword_idx].append(f"{var}({low}) := {value};")
                else:
                    subword_assignments[subword_idx].append(f"{var}({high} downto {low}) := {value};")
                ...
                while field.high >= accesswidth * subword_idx:
                    ...
                    if field.msb < field.lsb:
                        ...
                        sliced_value = do_bitswap(do_slice(value, f_high, f_low, reduce=False))
                    else:
                        sliced_value = do_slice(value, f_high, f_low, reduce=False)

                    addr = self._get_address_str(node, subword_offset=subword_idx*subword_stride)
                    var = self.get_readback_data_var(addr)
                    subword_assignments[subword_idx].append(f"{var}({r_high} downto {r_low}) := {sliced_value};")
...
class RetimedExtBlockReadbackMuxGenerator(ReadbackMuxGenerator):
    ...
    def process_external_block(self, node: AddressableNode) -> None:
        ...
        data = self.exp.hwif.get_external_rd_data(node)
        self.add_content(f"    readback_data_var := {data};")
        self.add_content("    is_external_block_var := '1';")
        self.add_content("end if;")
